...
class ODESolver:
    def enable_hicache(self, interval=4, max_order=1, first_enhance=2, end_enhance=None,
                       sigma=0.5, budget=None, max_horizon=None, max_memory_mb=None,
                       audit_budget=0):
        """HiCache (Hermite-polynomial velocity forecasting, arXiv:2508.16984) on the EULER
        solver — forecast the velocity on skipped steps instead of calling dynamics_fn.
        Training-free; native (no monkey-patching)."""
        self._hicache_cfg = dict(interval=interval, max_order=max_order, first_enhance=first_enhance,
                                 end_enhance=end_enhance, sigma=sigma)
        self._hicache_budget = budget or CacheBudget(
            backend="hermite", allowed_stages=("slat-flow",),
    ...
    def get_hicache_telemetry(self):
        """Return current or last-run cache telemetry, including unsupported-solver status."""
    ...
    def _make_hicache_runtime(self, state, x_init, times):
        ...
        return CacheBudgetRuntime(
            ...
            source_digest=stable_digest({"adapter": "fastsam3d-plus", "contract": "budget-v1"}),
        ...
        if decision == "forecast":
            ...
            if budget_runtime is not None:
                budget_runtime.record_measurement(
                    "slat-flow", "hermite", wall_time_ms=(time.perf_counter() - timer) * 1000.0,
                )
        else:
